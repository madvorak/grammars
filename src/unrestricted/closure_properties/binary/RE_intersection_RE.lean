...
def rule_step_right {T N₁ N₂ : Type} (s₁ s₂ : option (N₁ ⊕ T)) (x y z : N₂ ⊕ T) :
...
def rule_step_right_end {T N₁ N₂ : Type} (s₁ s₂ : option (N₁ ⊕ T)) (x y : N₂ ⊕ T) :
  frule T (super_nonterminal T N₁ N₂) :=
frule.mk
...
def rule_step_left {T N₁ N₂ : Type} (s₁ s₂ : option (N₁ ⊕ T)) (x : N₂ ⊕ T) :
  frule T (super_nonterminal T N₁ N₂) :=
frule.mk
...
def rule_step_left_end {T N₁ N₂ : Type} (s₁ s₂ : option (N₁ ⊕ T)) :
  frule T (super_nonterminal T N₁ N₂) :=
frule.mk
...
def rule_finalize_terminal {T N₁ N₂ : Type} (t : T) :
  frule T (super_nonterminal T N₁ N₂) :=
frule.mk
...
def rule_delete_empty_empty {T N₁ N₂ : Type} :
  frule T (super_nonterminal T N₁ N₂) :=
frule.mk
...
def all_used_terminals_f {T : Type} (g : frammar T) : list T :=
list.filter_map as_terminal (list.join (list.map frule.output_string g.rules))

def rules_finalize_terminal {T : Type} (g₁ g₂ : frammar T) :
...
def all_used_nonterminals_f {T : Type} (g : frammar T) : list g.nt :=
sorry

def rules_step_left_end {T : Type} (g₁ g₂ : frammar T) :
  list (frule T (super_nonterminal T g₁.nt g₂.nt)) :=
let z := option.none ::
