import unrestricted.general_grammar
...
def rule_step_right {N₁ N₂ : Type} (s₁ s₂ : option (N₁ ⊕ T)) (x y z : N₂ ⊕ T) :
  ggrule T (super_nonterminal T N₁ N₂) :=
ggrule.mk
...
def all_used_terminals_f (g : ggrammar T) : list T :=
list.filter_map as_terminal (list.join (list.map ggrule.output_string g.rules))

def rules_finalize_terminal (g₁ g₂ : ggrammar T) :
  list (ggrule T (super_nonterminal T g₁.nt g₂.nt)) :=
...
def all_used_nonterminals_f (g : ggrammar T) : list g.nt :=
sorry

def rules_step_left_end (g₁ g₂ : ggrammar T) :
  list (ggrule T (super_nonterminal T g₁.nt g₂.nt)) :=
let z := option.none ::
